...
void fk(const vector<double>& q, double (*p)[4]){
    ...
    p[1][0] = p[0][0] + L1 * cos(q[0]);
    p[1][1] = p[0][1] + L1 * sin(q[0]);
    p[1][2] = q[0];

    //p2: pose of joint #3 (end of link 2)
    p[2][0] = p[1][0] + L2 * cos(q[0] + q[1]);
    p[2][1] = p[1][1] + L2 * sin(q[0] + q[1]);
    p[2][2] = q[1];

    //p3: pose of the tool end (end of link 3)
    p[3][0] = p[2][0] + L3 * cos(q[0] + q[1] + q[2]);
    p[3][1] = p[2][1] + L3 * sin(q[0] + q[1] + q[2]);
    p[3][2] = p[2][2] + L3 * sin(q[1] + q[2]);
}
